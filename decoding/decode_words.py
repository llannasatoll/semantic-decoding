    ...
    for story in test_stories:
        ...
        if save_location:
            ...
            if "Llama" in str(em_data['model_path']):
                strs = [[[gpt.tokenizer.decode(ii) for ii in gpt.tokenizer.encode(s.decode()) if ii != gpt.tokenizer.bos_token_id] for s in data["can_stcs"][jj]] for jj in range(len(data["can_stcs"]))]
                chance_strs = [[[gpt.tokenizer.decode(ii) for ii in gpt.tokenizer.encode(s.decode()) if ii != gpt.tokenizer.bos_token_id] for s in data["chance_stcs"][jj]] for jj in range(len(data["chance_stcs"]))]
