...
from utils_ridge.SemanticModel import SemanticModel
from utils_ridge.dsutils import make_semantic_model_old, make_word_ds, make_phoneme_ds
from utils_ridge.stimulus_utils import load_textgrids, load_simulated_trfiles

from config import REPO_DIR, DATA_LM_DIR, DATA_TRAIN_DIR
...
def get_story_phonseqs(stories):
	grids = load_textgrids(DATA_TRAIN_DIR, DATA_TRAIN_DIR)
...
def get_eng1000_vectors(allstories):
	...
	Returns:
		Dictionary of {story: downsampled vectors}
	"""
	eng1000 = SemanticModel.load(join(DATA_LM_DIR, "english1000sm.hf5"))
	wordseqs = get_story_wordseqs(allstories)
	vectors = {}
	for story in allstories:
		sm = make_semantic_model_old(wordseqs[story], [eng1000], [985])
