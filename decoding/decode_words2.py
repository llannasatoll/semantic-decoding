    ...
    for story in test_stories:
        ...
        for i in range(len(word_rates)-unfixed_tr):
            ...
            for words_corr_list in candidate:
                ...
                num_oldtok = len(gpt.tokenizer.encode(blank.join(words[-1000:])))
                for ii, sample_output in enumerate(outputs):
                    tmp = [gpt.tokenizer.decode(t, skip_special_tokens=True) for t in sample_output[num_oldtok:]]
            ...
            for chance in chances:
                ...
                num_oldtok = len(gpt.tokenizer.encode(blank.join(words[-1000:])))
                tmp = [gpt.tokenizer.decode(t) for t in outputs[num_oldtok:]]
                if len(tmp) != num_words:
